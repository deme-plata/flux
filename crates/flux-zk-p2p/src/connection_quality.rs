...
use flux_zk_types::{ConsensusVote, ProposalHash};
use flux_zk_snark::{CircuitBuilder, SNARKConfig, SNARKProtocol, UniversalSNARK};
...
mod tests {
    ...
    // test helper: pack a label into a 32-byte ProposalHash (`[u8;32]` has no `From<&str>`)
    fn ph(s: &str) -> ProposalHash {
        let mut h = [0u8; 32];
        let b = s.as_bytes();
        let n = b.len().min(32);
    ...
    async fn test_consensus_participation_proof() {
        let voting_history = vec![
            ConsensusVote {
                epoch: 100,
                proposal_hash: ph("proposal1"),
                participated: true,
            },
            ConsensusVote {
                epoch: 101,
                proposal_hash: ph("proposal2"),
                participated: true,
            },
            ConsensusVote {
                epoch: 102,
                proposal_hash: ph("proposal3"),
                participated: false,
            },
        ];
