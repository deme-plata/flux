...
impl LatticeGuardProver {
    ...
    pub fn generate_proof<R: Rng + CryptoRng>(
        ...
        for (i, constraint) in circuit.constraints.iter().enumerate() {
            ...
            // Encode each constraint value as a CONSTANT polynomial: the value
            // in the constant term, zeros elsewhere, spanning the full ring
            // dimension. (BUGFIX: the old `vec![v; dim.min(64)]` both mis-sized
            // the NTT — len 64 != dimension — and, repeated across all coeffs,
            // blew the RLWE error bound. A constant term + zeros is the correct,
            // low-error, full-dimension encoding; verifier.rs matches.)
