//! Prover side: build the windowed FRI proof using `flux_zk_stark::StarkSystem`.
//!
...
impl WindowBuilder {
    ...
    pub async fn new(anchor_height: u64, anchor_state: StateRoot) -> anyhow::Result<Self> {
        let stark = flux_zk_stark::StarkSystem::new(false).await?;
        let anchor_chain = flux_recursive_proofs::tip_anchor(anchor_height, anchor_state);
        Ok(Self {
    ...
    pub async fn extend(&mut self, step: HeaderChainStep) -> Result<TipProofStir, VerifyError> {
        ...
        if self.window.len() >= WINDOW_SIZE {
            ...
            self.anchor_chain = flux_recursive_proofs::tip_extend(
                &self.anchor_chain,
                new_height,
                new_state,
...
pub async fn anchor(anchor_height: u64, anchor_state: StateRoot) -> anyhow::Result<TipProofStir> {
    let anchor_chain = flux_recursive_proofs::tip_anchor(anchor_height, anchor_state);
    let mut stark = flux_zk_stark::StarkSystem::new(false).await?;
