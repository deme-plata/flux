...
use flux_zk_types::{ConsensusVote, ValidatorId};
...
mod tests {
    ...
    async fn test_zk_p2p_manager_creation() {
        let config = ZkP2pConfig::new(
            flux_zk_types::validator_id_from_label("test_validator"),
            1000000, // 1M stake
            85,      // 85 reputation
        );
    ...
    async fn test_bootstrap_proof_generation() {
        let config = ZkP2pConfig::new(flux_zk_types::validator_id_from_label("test_validator"), 1000000, 85);
