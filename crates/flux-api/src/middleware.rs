...
pub struct MiddlewareSpec {
    ...
    /// JSON field on a list response that holds the page's items. `None` means
    /// the Stripe/Stainless convention `"data"`. Only meaningful when
    /// `pagination` is active — a paginator has to know which key to walk.
...
/// Page size a generated `Offset` paginator requests per round-trip. Kept as a
/// named constant so every language emits the same number.
pub const DEFAULT_PAGE_SIZE: u32 = 100;

impl PaginationStyle {
    pub fn is_active(&self) -> bool {
        !matches!(self, PaginationStyle::None)
    }
}

impl StreamKind {
    /// True only for SSE. WebSocket surfaces are generated by
    /// `event_types`, not by the REST paginator/stream emitters.
    pub fn is_sse(&self) -> bool {
        matches!(self, StreamKind::Sse)
    }
}
...
impl MiddlewareSpec {
    ...
    /// Override the list-response items key (default `"data"`).
    pub fn with_items_field(mut self, field: impl Into<String>) -> Self {
        self.items_field = Some(field.into());
        self
    }
    /// The items key a generated paginator should walk. Falls back to the
    /// Stripe/Stainless convention so an un-configured spec still emits
    /// working code rather than nothing.
    pub fn items_key(&self) -> &str {
        self.items_field.as_deref().unwrap_or("data")
    }
...
mod tests {
    ...
    #[test]
    fn items_key_defaults_to_data() {
        // An un-configured paginated spec must still emit working code, so the
        // accessor falls back rather than returning an empty key.
        assert_eq!(MiddlewareSpec::default().items_key(), "data");
        assert_eq!(
            MiddlewareSpec::default().with_items_field("results").items_key(),
            "results"
        );
    }

    #[test]
    fn pagination_and_stream_activity_probes() {
        assert!(!PaginationStyle::None.is_active());
        assert!(PaginationStyle::Page { page_param: "page".into() }.is_active());
        assert!(!StreamKind::None.is_sse());
        assert!(StreamKind::Sse.is_sse());
        // WebSocket is handled by event_types, not the REST stream emitter.
        assert!(!StreamKind::WebSocket.is_sse());
    }
