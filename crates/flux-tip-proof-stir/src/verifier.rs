...
/// 4. Inner FRI proof verified by `flux_zk_stark::StarkSystem::verify`.
pub async fn verify(
    proof: &TipProofStir,
    ...
    flux_recursive_proofs::tip_verify(
        &proof.anchor_chain,
    ...
    // ─── 4. Inner FRI proof ──────────────────────────────────────────────────
    // flux_zk_stark::verify needs a mutable StarkSystem (for perf monitor),
    // but the verify path itself is read-only on the proof. Spin up a
    // throwaway system per verify — cheap, ~1 ms.
    let mut stark = flux_zk_stark::StarkSystem::new(false)
        .await
